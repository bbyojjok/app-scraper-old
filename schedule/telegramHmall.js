...
const token = process.env.TELEGRAM_BOT_TOKEN_HMALL;
const bot = new TelegramBot(token, { polling: true });

let alertJob;
const hmallNewReviews = [];

const setHmallNewReviews = (name, data, result) => {
...
const getAlertReview = async chatId => {
  ...
  for (let i = 0, len = hmallNewReviews.length; i < len; i++) {
    ...
    const imageHmallUrl = `http://review.hdmall.com/images/icon-hmall-telegram-${os}.png?ver=1`;
...
module.exports = { setHmallNewReviews };
