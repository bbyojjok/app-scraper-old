...
const sendNewReviews = async (chatId, { newReviews, name, bot }) => {
  // 건별 텔레그램 메시지 전송
  for (let i = 0, len = newReviews.length; i < len; i++) {
    ...
    console.log(`[TELEGRAM] #${name} send newReviews`, caption);
  }
...
for (let i = 0, len = telegramSites.length; i < len; i++) {
  ...
  bot.onText(/\/start$/, (msg, match) => {
    ...
    alertJob = schedule.scheduleJob('30 8 * * *', () => sendNewReviews(chatId, site));
    bot.sendMessage(chatId, '평점 1점 앱리뷰 알림을 시작합니다.\n(매일 오전 8시 30분)');
    console.log(`[TELEGRAM] #${name} 평점 1점 앱리뷰 알림을 시작합니다. (매일 오전 8시 30분)`);
  });
